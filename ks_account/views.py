...
class UsernameView(FormView):
    ...
    def form_valid(self, form):
        ...
        if exist_username:
            ...
            #                  extra_tags='from_registration')
            message_tags = f'from_registration username={user.username}|||mobile={user.mobile}'
            messages.success(self.request, 'ثبت نام با موفقیت انجام شد.', extra_tags=message_tags)
            ...
            if user is not None:
                if not user.is_active:
                    ...
                    if is_password_correct:
                        if remember_me:
                            # but the purpose is checking remember me checkbox is checked or not.
                            request.session.set_expiry(60 * 60 * 24 * 30)  # Here we extend session for 30 days.
                        else:
